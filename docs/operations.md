...
**절차를 적어 두지 않으면 사고 난 날에 발명하게 된다.** 네 문서가 `이벤트는 백업 대상`이라고
적고, 그 말을 실행하는 명령이 여기 있다.
...
**이 순서는 실제로 돌려 확인한 것이다** — 이벤트가 그대로 돌아오고 시퀀스도 따라온다.
...
파싱돼 셸이 그 줄에서 죽는다 — 그래서 project 를 변수로 둔다. 펜스 전체가 `bash -n` 을 통과한다.
...
- **`test -s` 와 그것을 잇는 `&&`** — 빈 파일은 SQL 오류가 아니라 **문장이 0개인 실행**이라
  ...
  **`test` 를 따로 한 줄에 두면 이 가드는 아무 일도 하지 않는다.** 종료 코드를 소비하는 것이
  없어서다 — 실측: 0바이트 덤프에 대고 이 절이 적어 두었던 옛 블록을 그대로 돌리니
  `test -s` 가 `1` 을 냈는데도 `TRUNCATE` 가 이어서 돌아 **이벤트 셋이 0 이 됐다.**
  **가드가 `1` 을 냈는데 아무도 그것을 보지 않았다** — 뒤의 둘(`TRUNCATE`·붓기)이 `0` 으로 끝나
  전체가 성공처럼 보였다. 고친 블록은 같은 빈 덤프에서 `TRUNCATE` 를 아예 실행하지 않고
  종료 코드 `1` 로 멈추고, 진짜 덤프에서는 `COPY 3` 과 `setval` 뒤에 행 수를 찍는다
...
**이 프로세스는 요청을 직렬로 처리한다.** 라우트가 `async def` 인데 그 안에서 DB 호출이
동기로 일어나므로, 한 요청이 끝날 때까지 이벤트 루프가 잡혀 있다. 동시에 들어온 요청은
병렬로 돌지 않고 줄을 선다 — 벽시계 시간이 요청 수에 비례해 늘어난다 (실측).

- **에이전트가 도구를 병렬로 부르면** 그 대기가 그대로 보인다. 도구 호출을 겹쳐 던져도
  빨라지지 않는다
- **`POST /v1/ingest` 는 그중 가장 긴 요청이다.** 색인이 도는 동안 그 인스턴스의
  다른 표면은 답하지 못한다. 운영자 진입점이 CLI 인 이유이기도 하다 (D20) —
  `docker compose exec` 는 별도 프로세스라 서버를 잡지 않는다
- **여러 저장소를 섬기려면 스택을 복제한다** (D66). 같은 프로세스에 부하를 더 얹는 길은 없다

수치는 여기 적지 않는다 — 측정은 원장에 있다 (`module=verification`).
v1 은 이 동작을 바꾸지 않는다. 바꾸려면 그것은 새 D 다.
