...
**단계를 막는 Q 는 하나도 남지 않았다 — 1–10단계를 이제 검증할 수 있다.**
검증할 수 있다는 것은 **계약이 다 있다**는 뜻이다. 진행 상태는 §9가 소유하고, 열 단계가 다 구현돼 있다.
어느 Q 가 어느 D 로 닫혔는지는 [open-questions.md](open-questions.md)가 소유한다 —
그 이력의 사본을 여기 두지 않는다. 여기가 소유하는 것은 **아래 한 줄**, 즉 어느 단계가 어느 Q에 걸리는가다.
공백은 단계별로 걸린다 — **4단계 전에 Q16·Q18·Q21**, 5단계 전에 Q6·Q7·Q10,
...
`OPENAI_API_KEY` 를 넣고 한 번 돌려 D31·D33 이 `사람이 한 번 돌려 확인한다` 로 남겨 둔 자리를 채웠다.
커밋된 구성은 그대로다: `--profile test` 는 키를 갖지 않는다 (D33).

**측정치는 이 문서에 두지 않는다 — 이벤트 원장에 있다.** run 번호·점수·비는
[SKILL](skills/sillok-storage/SKILL.md) 의 결정 트리가 이벤트로 분류하는 `실험·벤치 결과`다.
게다가 이 파일은 색인 대상이라 여기에 적으면 그 문서의 청크가 다시 만들어져 **벡터 순위가 움직인다** —
값을 적는 행위가 그 값을 낡게 만든다. `search_events` 로 읽는다:
`project=sillok` 의 `module=ingest`(D31 백필, 이벤트 `137911`) 와
`module=search`(D33 검색 거동, 이벤트 `137912`).

여기 남는 것은 **키가 있을 때 지금 참인 것**뿐이다.

- **두 팔이 다 찬 RRF 가 돈다** (D33). 점수는 두 순위의 `1/(60+a) + 1/(60+b)` 로 분해된다 —
  병합 공식이 문서 그대로라는 뜻이다
- **`score` 는 임계값이 아니다.** 색인 상태만 바뀌어도 움직이므로 **한 응답 안에서만** 비교한다 (D33)
- **`RRF_K = 60` 은 점수를 납작하게 만들고 순서를 수렴시킨다** — `k` 를 60 위로 올려도 순서가 같고,
  20 이하로 내리면 달라진다. 한 팔에서만 상위인 행이 올라오는 것을 누르는 값이다
- **키가 있으면 `search_docs` 는 필터 없이 0건을 내지 않는다.** 벡터 팔에 거리 임계값이 없어
  뜻 없는 질의도 후보를 채우고, **필터가 집합을 비울 때만** 0건이다.
  그 신호를 대신할 것은 만들지 않는다 (D65)
- **키 없이 재색인하면 이미 있던 벡터가 사라진다** (D31). 한때 `아직 안 본 것`이었는데
  키가 들어와 벡터가 생긴 뒤 관측했다. **사라지는 것은 바뀐 문서의 것뿐이다** —
  재색인이 `(project, repo, path)` 단위로 청크를 지우고 다시 넣으므로 그 문서의 벡터가 함께 없어지고,
  키가 없으면 마지막 백필 패스가 채우지 못한다. 바뀌지 않은 문서의 벡터는 그대로 남는다.
  그 run 은 실패가 아니라 **성공이고 백필만 무동작**이다 — 빈 문자열 키는 인증 실패로 갈라지므로
  부재와 섞지 않는다. 수는 `search_events` 로 읽는다: `project=sillok` 의 `module=ingest`
...
> **이 블록은 목표가 아니라 도는 명령이다.** 이 순서로 돌리면 1번째가 `배치 검증 통과`,
> 2번째가 `db`·`api` 둘 다 `Healthy`, 3번째가 `ok` 로 끝난 run 하나, 4번째가 `{"ok":true,…}` 봉투,
> 5번째가 스모크 전부 `PASS`, 6번째가 호스트에서 DB 검사를 skip 한 채 통과다.
> 수치는 여기 적지 않는다 — 위 체크박스가 같은 이유로 파일 수를 적지 않는다.
> 5번째 줄이 10단계 스모크다 (D53). 6번째 줄을 전부 돌리려면 D22의 `--profile test`다.
> `exec api` 가 `sillok` 을 찾는 것은 `Dockerfile` 의 runtime 스테이지가 `/app/.venv/bin` 을
> PATH 에 넣기 때문이다. 마이그레이션을 더한 뒤에는 `api` 이미지를 다시 굽는다 —
