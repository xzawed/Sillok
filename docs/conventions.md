...
- **결정의 선택지 표** — D30 이후의 `## Dnn` 절이 `선택지`나 `버린 안` 표를 갖는지 (D62).
  D16–D29는 규칙보다 앞서므로 **면제 목록에 이름으로** 적는다 — 조용히 건너뛰지 않는다
- **`status`의 생애** — `superseded`는 `superseded_by`(실재하고 색인되는 경로),
  `stale`은 `stale_since`(날짜)를 front matter에 요구한다 (D61). **DB에는 `status`만 간다**
