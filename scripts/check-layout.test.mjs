...
const CASES = [
  ...
  },
  {
    ...
    // **줄이 아니라 문단을 본다는 것**을 잠근다. 열거가 틀렸는데 닫는 줄에 D 가 하나도
    // 없으면, 줄만 보는 창은 `mentioned` 가 비어 **조용히 통과한다** — 거짓 양성이 아니라
    // 침묵이라 더 나쁘다. 창을 줄로 되돌리면 이 케이스가 붉어진다.
    id: '66 닫는 줄에 D 가 없어도 끊긴 열거를 잡는다',
    expect: 'fail',
    mentions: ['임의로 뒤집지 않는다', 'D53 에서 끊긴다'],
    mutate: (dir) =>
