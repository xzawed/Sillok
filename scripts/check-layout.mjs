...
const RETIRED = [
  ...
  // 2026-09-06 문서 감사가 찾은 것. 고쳐 쓴 문장만 등록한다 —
  // 옮기기만 한 문장(plan.md §9 의 실측)은 **참인 채로 자리를 옮겼으므로** 등록하지 않는다.
  // 한 문장이 자기와 모순됐다: 같은 줄이 `test -s` 를 1 이라 해 놓고 전부 0 이라 했다.
  ['이벤트 셋이 0 이 됐고 종료 코드는 전부', '가드는 1 을 냈다 — 0 은 뒤의 둘뿐이다'],
  // 진입 문서가 열린 질문의 **재고를 단언**했다. 열린 Q 는 0 이고 막힌 단계도 없다.
  // 역할 문장은 Q 가 열리고 닫혀도 참이어야 한다 — 그래서 존재 함의를 뺀 문장으로 고쳤다.
  ['Open design questions are tracked', '열린 Q 는 0 이다 — 재고를 단언하지 않는다'],
  ['미해결 설계 질문은', '열린 Q 는 0 이다 — 재고를 단언하지 않는다'],
  // 측정치가 원장으로 옮겨 가면서 ADR 의 포인터 둘이 낡았다. 사본이 낡는 그 부류다.
  ['기록은 [plan.md](../docs/plan.md) §9', '측정치는 이벤트 원장에 있다 (137911·137912)'],
]
