...
const RETIRED = [
  ...
  // 같은 감사가 고친 나머지 셋은 **등록할 수 없다** (Grok 적대 리뷰가 잡았다).
  // RETIRED 는 `지금 거짓인 문구` 의 목록인데 셋 다 상태에 따라 참이 된다 —
  // `Open design questions are tracked` 와 `미해결 설계 질문은` 은 Q34 가 열리는 날 참이고,
  // 뒤의 것은 접두사라 `미해결 설계 질문은 없다` 라는 참인 문장까지 문다.
  // `기록은 [plan.md](…) §9` 도 §9 가 여전히 소유하는 것(v1 완료 조건)에 쓰면 참이다.
