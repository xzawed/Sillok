...
const RETIRED = [
  ['키가 있으면 벡터 팔만 남은', 'D65 의 조건은 키가 아니라 문서 벡터다'],
  ['키가 있으면 D33이 지정한', 'D65 의 조건은 키가 아니라 문서 벡터다'],
  ['키가 있으면 search_docs 는 필터가 집합을', 'D65 의 조건은 키가 아니라 문서 벡터다'],
  ...
  ['잠금 파일을 조용히 되돌리므로', 'uv run 은 돌기 전에 잠금과 환경을 맞춘다 — 이유가 달랐다'],
  ...
  ['D54 이후로', '다음 번호는 D65 이다'],
  ['그 블록은 목표이고', '§9 의 여섯 줄은 2026-09-03 에 다 돌렸다'],
  ['증거 3종', '개수를 적지 않는다 — 셋이던 것이 여섯이 됐다'],
  ...
  // 2026-09-06 문서 감사가 고친 넷은 **하나도 등록하지 않는다** (Grok 적대 리뷰가 두 번 잡았다).
  // 옮기기만 한 문장(plan.md §9 의 실측)은 참인 채로 자리를 옮긴 것이라 애초에 대상이 아니고,
  // 나머지 넷은 전부 **문맥에 따라 참이 된다** — RETIRED 는 `지금 거짓인 문구` 의 목록이다:
  //   `Open design questions are tracked` · `미해결 설계 질문은` — Q34 가 열리는 날 참이고,
  //     뒤의 것은 접두사라 `미해결 설계 질문은 없다` 라는 참인 문장까지 문다
  //   `기록은 [plan.md](../docs/plan.md) §9` — §9 가 여전히 소유하는 것(v1 완료 조건)에
  //     쓰면 참이다. **말줄임표로 줄여 적지 않는다** — 대조군이 무는 것은 이 글자 그대로다
  //   `이벤트 셋이 0 이 됐고 종료 코드는 전부` — `…전부 0 이 아니었다` 로 닫으면 참이다.
  //     완결된 명제가 아니라 접두사라, 정정 문장이 그대로 걸린다
