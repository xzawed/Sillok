...
- **`status: "current"` 는 기본값이 아니다.** [service-and-mcp.md](../docs/service-and-mcp.md) 요청 예시의
  ...
  → **D61 이 닫았다** — 무엇이 언제 `stale` 이 되는지 정했다
...
| `search_events` 의 `summary` | 그렇다 | 800자 + `…` — `excerpt` 와 같은 천장이다 (D33) |
| `save_event` 의 `payload` | 그렇다 (`get_event` 가 행을 돌려준다, D39) | `json.dumps(payload, ensure_ascii=False)` 가 2000자 이하. **`summary` 와 같은 숫자**이고 같은 이유다 |
...
뜻하게 된다. 새 정수가 천장이 떨어뜨린 키 수를 말하므로 그 차이가 계속 읽힌다.
...
|---|---|---|
| `current` | 지금 규범. 기본값 | — |
| `draft` | 아직 계약이 아니다 | README 에서 도달 가능해야 하고, `search_docs` 기본 필터에서 빠진다 |
| `superseded` | 다른 문서가 그 자리를 대신했다 | `superseded_by: <레포 상대 .md 경로>` — 실재하고 그 자신도 색인 대상이어야 한다 |
| `stale` | 사실이 낡았는데 아직 못 고쳤다 | `stale_since: YYYY-MM-DD` |
...
D33 의 기본 필터 `status: "current"` 가 그제야 뜻을 갖는다.
...
- 헤더에 `본문 해시: sha256:<앞 12자리>` 가 생긴다. 대상은 헤더 블록 아래 전부다
