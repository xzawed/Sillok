---
title: v1 확정 결정 D1–D46
doc_type: adr
status: current
module: null
---

# ADR 0001 — v1 확정 결정 D1–D46
...
· D42–D46 **2026-09-02 확정** (8단계 MCP 표면 — **마지막 단계 게이트를 닫았다**)
...
- **`kb_query_logs` 기록(9단계)은 이 결정들 밖이다**
...
그래서 검증 계층은 하나로 남는다 — 단, **타입을 어긴 호출은 예외다.**
값의 타입이 스키마와 다르면 SDK가 도구 실행 전에 오류 결과로 답하고, 그 본문은 봉투가 아니다(실측).
...
D47 이후로 기록해야 할 미해결 결정은 [docs/open-questions.md](../docs/open-questions.md)에 전부 모여 있다.
2026-09-02 기준 남은 것은 C절의 Q13·Q14 · D절의 Q22·Q23 · Q24·Q25 · G절의 Q30·Q31이다.
**Q17이 D42–D46으로 닫히면서 단계를 막는 Q는 하나도 남지 않았다** — §7의 게이트 문장은 그대로 둔다.
...
7단계를 막던 Q12·Q15·Q19·Q20은 D35–D38로 닫혔다 — 남은 단계 게이트는 **8단계의 Q17** 하나다.
F절(Q27–Q29)은 7단계를 구현하다 열렸고 같은 날 D39–D41로 닫혔다 — 열린 채로 코드가 나간 적은 없다.
**G절(Q30·Q31)은 그 7단계를 검증하다 열렸고 아직 답이 없다** — `GET /v1/docs`의 단계·계약과
D30 전체 스캔의 비용이다. 둘 다 단계를 막지 않으므로 §7 의 게이트 문장에는 넣지 않는다.
