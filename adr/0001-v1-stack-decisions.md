...
- **디코드 실패의 시점.** 읽기·해시·청크·임베딩은 파일 하나 안에서 끝나고 그 뒤에 트랜잭션이 열린다(D32).
...
- run 행은 서로 독립된 쓰기다 — 시작 INSERT(`running`) → 파일 루프 → 백필 → 종료 UPDATE.
...
- `POST /v1/ingest` 는 `partial` 에도 `ok: true` 와 `data.status` 를 돌려준다.
  봉투의 `ok: false` 는 요청이 처리되지 못했다는 뜻이고 부분 run 은 처리된 것이다.
  대신 **`status` 를 안 읽는 클라이언트는 부분 실패를 놓친다** — 운영자 진입점이 CLI 인 이유다 (D20).
