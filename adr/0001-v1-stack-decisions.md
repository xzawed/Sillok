---
title: v1 확정 결정 D1–D34
doc_type: adr
status: current
module: null
---

# ADR 0001 — v1 확정 결정 D1–D34
...
| ID | 선택 | 결정 내용 | 닫은 질문 |
|---|---|---|---|
| D33 | A | **두 순위를 RRF(`k=60`)로 합친다.** 각 순위는 완결된 정렬 위의 `rank()` 이고, 키워드 순위는 `plainto_tsquery('simple', …)` + `ts_rank(…, 1)` 이다. `score` 는 그 RRF 값이며 **한 응답 안에서만** 비교된다. `top_k` 는 청크 수를 세고 **한 문서는 최대 2행**까지 차지한다. 최종 정렬은 `score DESC, repo, path, chunk_idx` 로 총순서다 | Q8 |
...
| 질의 파서 | `to_tsquery` 는 사용자 입력에 **예외를 던진다** — D21이 그것을 `INTERNAL` 고정 문구로 접는다. 오타가 서버 장애로 보인다 | `plainto_tsquery` — 예외도 부정 연산자도 없다 |
...
```text
c.id ASC  -> 498,461,473,497,499,327,352,384
c.id DESC -> 498,499,497,473,461,510,509,507
random()  -> 498,461,497,473,499,384,495,327
```

세 답이 모두 같은 `ORDER BY` 를 만족한다. 앞 둘의 교집합은 여덟 중 셋이다.
...
매칭이 없는 텍스트에서 `ts_headline` 길이는 통제되지 않는다 — 세 청크에서 `1`·`309`·`116`자.
**1자짜리 발췌가 정상 출력이다.**
...
- **질의 파서는 `plainto_tsquery('simple', query)` 하나다.** 예외를 던지지 않고, 부정도 구문 검색도 없다.
...
- **낱말이 하나도 남지 않는 질의**(예: `---`)는 키워드 목록이 빈 것이다. 오류가 아니다.
  단, **공백만 있는 `query` 는 `VALIDATION`** 이다 — 임베딩 입력으로도 쓸 수 없는 값이라 두 경로가 모두 무의미해진다.
...
- **기본 8, 최대 12다. 범위 밖은 `VALIDATION` 으로 거절한다** — 조용히 12로 접지 않는다(D25 선례).
  `query` 는 필수이고 공백뿐이면 `VALIDATION` 이다.
...
- `search_events` 의 최종 타이브레이크는 **`id`** 다. 이벤트 행은 갱신되지도 재삽입되지도 않으므로
  `id` 가 안정한 키다. 문서 쪽과 다른 키를 쓰는 이유가 이것이다.
...
- **키워드로 걸리지 않은 행에는 `ts_headline` 을 쓰지 않는다.** 벡터로만 걸린 행과 낱말이 없는 질의가 그 경우다.
  매칭이 없으면 출력 길이가 통제되지 않는다 — 실측에서 1자짜리 발췌가 나왔다.
...
| `plainto_tsquery` | 사용자 오타가 `INTERNAL` 이 되거나 색인 전체가 매칭된다 | 6단계 검사 — `&`·`-단어`·`""` 입력이 예외 없이 지나는지 |
...
D34는 순서도 `score` 도 정하지 않는다. 대신 D33이 반드시 받아야 하는 사실 셋을 남긴다.
...
D35 이후로 기록해야 할 미해결 결정은 [docs/open-questions.md](../docs/open-questions.md)에 전부 모여 있다.
2026-09-01 기준 남은 것은 C절의 Q12–Q15·Q17 · D절의 Q19·Q20·Q22·Q23과 Q24·Q25다.
