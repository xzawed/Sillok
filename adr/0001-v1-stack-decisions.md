...
· D53 **2026-09-03 확정** (10단계 스모크의 형태와 자리)
· D54–D57 **2026-09-03 확정** (운영 — 백업·재기동·검사 DB·CI·기여)
· D58–D64 **2026-09-03 확정** (단계 밖에 남아 있던 열린 Q 일곱)
...
- ~~테스트가 제품 `db_data` 볼륨을 함께 쓴다. 격리는 정하지 않았다~~
  → **D55 가 닫았다** — 같은 볼륨을 쓰고 격리는 `t_` 접두사로 하며 검사가 그것을 지킨다
...
- ~~기여 절차·이슈 템플릿·행동 강령은 여전히 없다. 필요해지면 그때 결정한다~~
  → **D57 이 닫았다** — 받지 않기로 정했다 (2026-09-03)
- ~~두 README가 갈라지는 것을 막는 검사는 아직 없다~~ → **2026-09-03 에 게이트의 검사 16 이 생겼다.**
  구조(`##`·`###`·표·행·펜스)를 대조한다. **사본이 낡는 것은 이 저장소의 알려진 실패 모드다**
...
- **Q22 · Q23 · Q25** `repo` 의미, front matter 규칙(**절반은 D29 가 답했다**), Skill 사본 노후
  → **D60·D61·D63 이 셋 다 닫았다** (2026-09-03)
...
- **저장과 검색이 서로 다른 규칙을 쓴다.** 저장은 받은 그대로(D25)이고 검색 필터는 트리밍한다(D33 §1).
  그래서 `module` 을 공백째 저장하면 트리밍된 필터로는 그 행을 찾지 못한다. 규칙 둘 다 의도된 것이고,
  **둘이 만나는 자리를 이 결정이 다루지 않는다**
...
- **pytest 는 더 이상 `COPY src`·`COPY tests`·`COPY migrations` 를 목격하지 않는다.** 이미지가 소스를 제대로
  ...
  → **2026-09-03 에 그 둘도 마운트한다** — 아래 항목이 그 이유다
- ~~의존성 변경 시 재빌드를 강제하는 검사는 없다~~ → **2026-09-03 에 검사가 생겼다.**
  `tests/test_dependencies.py` 가 **설치된 것 전부**를 `uv.lock` 과 대조하고, compose 의 `test`
  서비스가 잠금 파일을 마운트한다. 방향이 *설치 → 잠금* 인 것이 요점이다 —
  직접 선언한 것만 보면 `uv lock --upgrade` 가 전이 의존성만 올렸을 때 초록이고,
  `psycopg[binary]` 의 실체인 `psycopg-binary` 도 보이지 않는다 (적대 리뷰가 찍은 자리).
  반대 방향은 보지 않는다 — `pywin32`·`colorama` 처럼 이 플랫폼에 안 깔리는 것이 정상이다.
  실측 둘: 전이 의존성 `starlette` 를 갈라 놓으니 `설치 1.6.0 vs 잠금 ['9.9.9']`,
  `h11` 블록을 지우니 `잠금 파일에 없다 (지운 의존성이 남아 있다)` 로 붉어졌다.
  **이 검사가 증명하지 못하는 것**은 그 잠금 파일이 작업 트리의 것이라는 사실이다 —
  마운트를 지우면 이미지 안의 둘을 비교해 언제나 통과하고, 안에서는 알 수 없다
...
- **Q23 은 닫히지 않는다.** D29 는 네 필드를 *어디서* 얻는지만 정했다. `draft`·`superseded`·`stale` 을 **언제** 붙이는지는
  ...
  → **D61 이 닫았다** — `status` 넷의 조건과 두 증거 키를 정했다
...
- ~~두 README 가 갈라지는 것을 막는 검사는 여전히 없다~~ → **검사 16 이 그 자리를 채웠다** (2026-09-03)
- **Q22** (`kb_documents.repo` 의 의미) 는 이 결정과 무관하게 열려 있다
  → **D60 이 닫았다** — v1 은 `''` 로 고정하고 그것이 규칙이 된다
...
> **2026-09-16 전수 대조: 그 셋은 다 생겼다.** 확장자는 `check-index-parity.mjs` 가 게이트 목록과
> 맞추고, 줄끝 해시는 호스트와 DB 양쪽에 있으며, `heading_path` 구분자는 6단계 소비자와 함께 왔다.
> 표의 `없다` 는 *그때* 없었다는 뜻이고 지금은 아니다 — 이 표를 지도로 읽는 사람을 위해 적는다.
...
- **전체 스캔의 비용을 재지 않았다.** 지금은 파일 열 개다. 수천 개가 되면 변경 목록 모드가 다시 후보가 되고,
  ...
  → **D47 이 그 전제를 뒤집었다** — 비용은 2026-09-02 에 쟀고(Q31), 그것을 정하는 것은
  색인 대상 수가 아니라 workspace 나무의 크기였다. `파일 열 개` 는 그래서 잘못된 자이고
  그 수도 이제 아니다 (개수는 게이트가 센다). D47 은 전체 스캔을 유지한 채 건너뛰는 목록을 늘렸다 —
  **변경 목록 모드는 D47 이 다룬 적이 없다.** 그것은 이 문단이 남겨 둔 그대로다
- **확장자 없는 `README` 는 조용히 색인되지 않는다.** 오늘 존재하지 않고 게이트도 같은 이유로 검사하지 않는다.
  게이트가 D9 경로 안의 *색인되지 않는 파일*을 목록으로 출력하게 하는 것이 다음 후보다
  (D29 가 면제 목록에 쓴 방식)
  → **그 후보는 들어왔다.** 게이트가 `색인 제외` 줄로 D9 경로 안의 먹지 않은 파일을
  사유(`not-md`·`symlink`)와 함께 출력한다 — 실패가 아니라 출력이다.
  확장자 없는 `README` 가 생기면 이제 조용히 빠지지 않는다
...
- **Q22 는 열려 있다.** `repo` 를 비워 두는 것은 의미를 정한 것이 아니라 정하지 않기로 한 것이다.
  ...
  → **D60 이 그 문장을 뒤집었다** — 비워 두는 것이 결정이다
...
- **[plan.md](../docs/plan.md) §9 의 `docker compose exec api sillok ingest` 가 그대로 도는지 확인하지 않았다.**
  ...
  → **2026-09-02 에 실행했고 예상대로 `executable file not found` 로 죽었다.** 같은 날
  `Dockerfile` 의 runtime 스테이지가 `/app/.venv/bin` 을 PATH 에 넣도록 고쳤다 —
  **위 두 줄은 그 이전 상태다.** 지금은 그 명령이 그대로 돌고,
  사실은 [plan.md](../docs/plan.md) §9 가 소유한다
...
- **키가 있는 상태의 검사 경로가 없다.** `--profile test` 는 키를 갖지 않으므로 백필 패스는
  ...
  → **2026-09-04 에 한 번 돌렸다.** `임베딩 320 · 남은 벡터 0` 이고 `바뀐 0` 인 run 에서 채워졌다
  (변경 판정과 무관하다는 이 결정의 요점 그대로). `kb_documents.indexed_at` 은 그대로였다.
  측정치는 **이벤트 원장**에 있다 (`search_events project=sillok module=ingest`, 이벤트 `137911`).
  [plan.md](../docs/plan.md) §9 `키가 있는 상태` 는 결론만 적는다. **검사 경로가 생긴 것은 아니다** —
  커밋된 구성은 여전히 무동작이고, 그것이 D33 이 의도한 바다
...
- **`status: "current"` 는 기본값이 아니다.** [service-and-mcp.md](../docs/service-and-mcp.md) 요청 예시의
  ...
  → **D61 이 그 물음에는 답했다** — 무엇이 언제 `stale` 이 되는지 정했다.
  **그래도 이 문장은 그대로다** — 기본값을 만들지 않는다 (D61 이 명시한다)
...
- **`query` 의 필수 여부는 엔드포인트마다 다르다.**
  ...
  - 값이 있는데 렉심이 하나도 나오지 않으면 **두 엔드포인트 모두 키워드 술어를 건다.**
    ...
    **결과는 엔드포인트마다 다르다.** `search_events` 는 목록이 하나뿐이라 0건이다 (D34 §3).
    `search_docs` 는 문서 벡터가 있으면 **벡터 팔만 남은 RRF** 이고, 없으면 두 목록이 다 비어 0건이다.
...
- **`repo` 는 오늘 전부 빈 문자열이지만 키에 넣는다.** Q22가 그 컬럼에 의미를 주는 날
  ...
  → **D60 이 그 날을 앞당겼다** — 의미는 `''` 이고 정렬 키에는 그대로 남는다
...
> **2026-09-16 전수 대조.** 이 표의 `plainto_tsquery` 행이 약속한 검사가 **트리에 없었다** —
> `&`·`-단어`·`""` 를 보내는 검사가 하나도 없었다. 같은 날 채웠고, 이벤트 쪽
> `websearch_to_tsquery` 행과 **같은 입력 목록**으로 잠갔다 (클라이언트는 파서를 고르지 않는다).
>
> `없다` 둘 중 **후보 풀 행의 이유는 낡았다** — 그때는 두 목록을 채우려면 키가 필요했는데,
> 지금은 검사가 `_embed` 를 가로채 벡터를 직접 넣는다. 만들 수 있는 자리가 됐고 아직 없다.
> `ts_rank` 정규화 행의 이유(`덜 맞는 것은 실패로 보이지 않는다`)는 그대로 참이다.
>
> **2026-09-17: 이 표의 검사 지시 둘이 낡았다.** 실측이 갈랐다.
> `필터로 배제될 문서를 상위에 놓고 결과 수를 본다` 는 고장을 못 본다 — 풀이 자르지 않으면
> 팔에서 걸러도 병합 뒤에 걸러도 결과가 같다. **풀보다 많이 쌓고 대상을 뒤로 보내야** 갈린다.
> `COLLATE "C"` 행의 `위와 같은 검사가 로케일 의존 문자를 포함한 경로로` 도 그렇다 —
> 최종 순서는 파이썬이 정하므로 그 검사로는 `COLLATE` 를 지워도 안 붉어진다.
> 그 절이 사는 자리는 **후보 풀을 고르는 `ORDER BY`** 이고, 검사도 그쪽으로 옮겼다.
...
- **`plainto_tsquery` 의 AND 가 재현율을 얼마나 깎는지 재지 않았다.** `임베딩 백필 경로` 가 0건인 것은
  ...
  → **2026-09-04: 그 신호가 `search_docs` 쪽에서 죽는다는 것이 실측으로 드러났다.**
  ~~키가 있으면~~ 벡터 팔이 필터 없는 모든 질의에 후보를 채우므로 `hit_count=0` 이 쌓이지 않는다.
  → **2026-09-15: 조건은 키가 아니라 문서 벡터다.** 키만 있고 `embedding` 이 NULL 이면 그 팔이 빈다 (D65).
  이 결정을 다시 볼 방법을 다시 정해야 한다 →
  [open-questions.md](../docs/open-questions.md) **Q33**
- **`RRF_K = 60` 을 이 색인에서 검증하지 않았다.** 목록이 하나뿐이라 `k` 는 순서에 아무 영향이 없다.
  두 목록이 다 차는 상태를 이 저장소는 아직 만들 수 없다 — 키가 필요하고 `--profile test` 는 키를 갖지 않는다
  → **2026-09-04 에 키를 넣고 쟀다.** 돌아온 여덟 행이 전부 `1/(60+a) + 1/(60+b)` 로 분해됐고,
  `k=60` 은 점수를 납작하게 만들면서(1위/8위 = `1.142`) **순서를 수렴시킨다** — 60·200·1000 이 같은 순서고
  20 이하가 다르다. 한 팔에서만 상위인 행을 누르는 값이다.
  측정치는 **이벤트 원장**에 있다 (`search_events project=sillok module=search`, 이벤트 `137912`).
  **`--profile test` 가 키를 갖지 않는 것은 그대로다** — 검증했다는 것이지 검사 경로가 생겼다는 뜻이 아니다
...
- **`score` 를 임계값으로 쓰는 소비자를 막을 장치가 없다.** 계약과 도구 설명이 말할 뿐이고,
  ...
  → **그 날이 2026-09-04 였다.** 키를 넣으니 1위가 `0.016393` 에서 `0.031545`(`run 11552`) 가 됐고,
  그 값을 [plan.md](../docs/plan.md) 에 적자 **그 문서가 다시 색인돼** `0.031319`(`run 11686`) 로 또 움직였다.
  임계값을 못 쓰는 이유의 실물이다 — 같은 질의의 1위 점수가 **색인의 상태만으로** 두 배가 되고,
  그 뒤에도 문장 하나에 흔들린다.
  **그래서 그 값은 이제 문서에 두지 않는다** — 위 서술의 값을 따라가려면 이벤트 `137912` 를 본다
- **페이지네이션은 없다**(Q13). `top_k` 를 넘는 결과를 보는 방법이 없다
  → **D58 이 그것을 결정으로 못 박았다** (v1 밖)
...
- **Q22가 열려 있다.** 정렬 키에 `repo` 를 넣었지만 그 값의 의미는 정해지지 않았다
  → **D60 이 닫았다** — 의미는 `''` 이고 응답에 싣지 않는다. 정렬 키는 그대로다
...
> **2026-09-16 전수 대조: 그 하나에도 검사가 생겼다.** `test_hnsw_is_absent_in_v1` 이 이름이 아니라
> 접근 방법(`pg_am`)으로 보므로 `kb_events_hnsw` 를 이름과 무관하게 잡는다. 이 행의 `없다` 는
> 이제 거짓이다 — 뒤에 온 일반 검사가 이 자리까지 덮었다.
>
> 같은 대조에서 **이 표의 약속 둘이 트리에 없는 것**도 드러났다 — 깨진 질의 두 줄(`plainto_tsquery`·
> `websearch_to_tsquery`)이다. 같은 날 채웠다. `상시 게이트` 줄도 §4 가 이름 붙인 연산자
> (`%`·`%>`·`<%`)를 안 찾고 있었고 그것도 같은 날 고쳤다.
...
- **Q12(프로젝트 경계)·Q13(페이지네이션)·Q22(`repo`)는 이 결정 밖이다**
  → **Q13 은 D58, Q22 는 D60 이 닫았다** (Q12 는 D35)
...
→ **D60 이 닫았다** — `''` 가 곧 의미다. 이 문단은 D37 시점의 상태다

### D37이 받아들이는 비용

여러 저장소의 **파일**까지 한 인스턴스로 섬길 수 없다. 그러려면 인스턴스를 여럿 띄운다.
~~막히는 것은 `get_file` 하나다~~ — **둘이다: `get_file` 과 `save_doc`.** 같은 날 정한 D38 의 `save_doc` 도
현재 본문을 같은 뿌리에서 읽는다 (`_read_current`). 이벤트 원장도 문서 검색도 여전히 멀티 프로젝트다 (D5).
...
`POST /v1/ingest` 는 그대로 MCP 밖이다 (D20). `GET /v1/docs` 는 표면 자체가 없다 (D64).
...
- **`GET /v1/docs`(Q30)** 는 여전히 열려 있다. MCP 에 노출하지 않으므로 8단계와 독립이다
  → **D64 가 닫았다** — 8단계와 독립인 채로 끝났다. 만들지 않는다
...
- **보존 규칙이 없다.** 자라는 것을 받아들인 것이지 해결한 것이 아니다 (D24 선례).
  ...
  → **2026-09-16: 이 항목은 닫히지 않았다.** 지운 것은 문서의 `최근` 한 마디뿐이다.
  미뤄 둔 것은 *보존 규칙이 생긴 뒤의 선택*이지 *오늘의 거동을 적는 문장*이 아니었고,
  그 문장이 구현과 어긋나 있었다. 이제 문서는 구현을 그대로 적는다 —
  그 자리의 정본은 같은 파일의 `zero_hit_queries는 kb_query_logs의 hit_count = 0 건수다` 다.
  **`최근` 인지 `전체` 인지는 여전히 정해지지 않았다**
- **`event_stats`·`get_*` 이 빠지므로 이 표는 `AI가 무엇을 물었나`의 전부가 아니다.** 검색만의 원장이다
- **`GET /v1/docs`(Q30)** 는 이 결정들 밖이다. 표면이 생기면 로그 대상인지 그때 정한다
  → **표면이 생기지 않는다** (D64). 그래서 로그 대상도 아니다

## D53 — 10단계 스모크의 형태와 자리 (2026-09-03 확정)

§7 10단계는 스모크가 **무엇을** 보는지 정했다 — 필수 필드 없는 이벤트 거절 · ingest 후 검색 · stats.
**어떻게 도는지는 어느 문서에도 없었다.** 구현이 조용히 채우면 그 임의가 나중에 계약처럼 인용된다
(D42–D46이 Q17에서 겪은 자리와 같다). 단계를 막지는 않으므로 Q 로 열지 않고 여기서 바로 정한다.

| ID | 결정 내용 | 닫은 질문 |
|---|---|---|
| D53 | 스모크는 `scripts/smoke.mjs` 다. **살아 있는 HTTP 표면만** 때리고 §9 판정 블록에 들어가되 `evidence.mjs` 에는 넣지 않는다 | 없음 (§7 10단계의 형태) |

- **왜 스크립트인가.** `curl -sf` 로는 첫 항목을 판정할 수 없다 — **거절을 기대하는데 그 옵션은 4xx 에서 죽는다.**
  상태코드와 봉투를 함께 보는 판정이 필요하고, 그것은 셸 한 줄이 아니다.
- **왜 HTTP 만인가.** CLI 경로는 §9 블록의 `compose exec api sillok ingest` 와
  `check-index-parity.mjs` 가 이미 태운다. 스모크가 보는 것은 *배포된 서비스가 도는가* 다.
- **왜 `evidence.mjs` 에 넣지 않는가.** 증거는 **PR 하나**의 것이고 스모크는 **v1 완료** 판정이다.
  넣으면 모든 PR 이 살아 있는 `api` 에 매인다 — 지금 증거는 `db` 만 있으면 돈다 (D22).
- **부작용은 ingest run 행 하나뿐이다.** project 는 `sillok` 이고, 거절은 저장하지 않으며 stats 는 읽기다.
  두 번째 run 이 아무것도 바꾸지 않는 것이 D30 이므로 반복해도 원장이 부풀지 않는다.
- **검색어는 project 이름과 다른 축이다.** 처음에는 질의를 project 이름으로 두었는데,
  고장 주입에서 *색인이 비었다* 와 *그 이름이 본문에 없다* 가 구분되지 않았다 (실측). `--query` 로 나눴다.
- **`ok: true` 는 색인이 성공했다는 뜻이 아니다.** 실패한 run 도 행이 생기면 봉투는 성공이고
  `status` 만 `failed` 다 (D32·D21). 스모크는 `status === 'ok'` 와 `files_seen > 0` 을 함께 본다 —
  보지 않으면 **실패한 색인 + 낡은 인덱스의 히트**가 통과로 나온다 (Grok 적대 리뷰가 잡았다).
- **주입은 커밋한다.** `scripts/smoke.test.mjs` 가 계약대로의 가짜 서비스를 세우고
  거기에 스모크를 태운다. DB 도 도커도 필요 없으므로 **그 주입만은 `evidence.mjs` 에 넣는다** —
  넣지 않는 것은 스모크 자신이지 스모크가 무는지 보는 검사가 아니다.
- **항목을 늘리려면 §7 을 먼저 고친다.** 스크립트가 셋보다 많이 보면 그 순간 §7 이 사본이 된다.

### D53 선택지

| 안 | 버린 이유 |
|---|---|
| `curl` 여러 줄을 §9 블록에 나열 | 거절 항목을 판정하지 못한다 — `-sf` 가 4xx 에서 죽고, 빼면 이번엔 실패를 통과로 읽는다. 봉투도 보지 않는다 |
| pytest 케이스로 만든다 | D22 의 `test` 프로파일 안에서 돌므로 살아 있는 `api` 를 때리지 못한다. 그 프로파일에는 `8080` 이 없다 |
| **`scripts/smoke.mjs`** | 채택. 게이트·증거와 같은 언어이고, 판정을 코드로 쓸 수 있으며, 실패하면 종료 코드가 1 이다 |

### D53 이 닫지 않는 것

- **스모크는 9단계 원장을 보지 않는다.** §7 이 셋만 말하고 이 결정은 §7 을 고치지 않는다 —
  그 자리는 §9 체크박스가 이미 갖고 있다
- **검색어 기본값 `Sillok` 은 이 저장소의 자기 색인을 전제한다.** 다른 workspace 에서는 `--query` 를 준다
- **이 스모크를 자동으로 도는 자리는 없다.** CI 가 없다 — 그 공백은 이 결정 밖이다
- **`SILLOK_BEARER_TOKEN` 이 걸린 배치에서만 헤더를 붙인다.** 토큰이 틀린 경우를 스모크가 따로 보지 않는다
- **부작용은 ingest run 행 하나가 전부가 아니다.** 9단계가 붙어 `search_docs` 가 질의 원장에도 한 행을 남긴다 (D48).
  둘 다 정상 운영과 같은 모양이라 받아들이지만, `kb_status` 의 수는 스모크를 돌린 만큼 자란다

## D54–D57 — 운영 (2026-09-03 확정)

§7 1–10단계가 끝나면서 **단계 밖에 남아 있던 공백**을 닫는다.
넷 다 "필요해지면 그때"로 미뤄져 있었는데, `이벤트는 백업 대상`이라고 **네 문서가 적어 두고
명령이 한 줄도 없는** 상태가 그 미룸의 대가였다.

| ID | 결정 내용 | 미뤄져 있던 자리 |
|---|---|---|
| D54 | 절차의 정본은 새 [docs/operations.md](../docs/operations.md)다. 백업 대상은 `kb_events` **하나**다 | 백업을 요구하는 네 문서, 명령 0건 |
| D55 | 검사는 제품과 **같은 DB·같은 볼륨**을 쓴다. 격리는 이름으로 하고 검사가 그것을 지킨다 | D22 `격리는 정하지 않았다` |
| D56 | **CI 를 두지 않는다.** 비밀 유출은 게이트의 검사 17 이 매번 본다 | D26 의 1회성 실측 |
| D57 | **외부 기여를 받지 않는다.** D15 의 `개인 도구`가 그대로다 | D26·D27 의 `필요해지면 그때` |

### D54 절차의 자리와 백업 대상

`doc_type: runbook` 은 taxonomy 에 선언만 되고 한 번도 쓰이지 않았다. 그 값이 이 파일이다.

**뜨는 것은 `kb_events` 뿐이다.** `kb_documents`·`kb_chunks` 는 Git 이 원본이고 `ingest` 가 다시 만든다.
`kb_ingest_runs` 는 색인 이력이지 지식이 아니고, `kb_query_logs` 는 v1 성공 조건의 *측정*이다 (D51).
전체 덤프는 재생성 가능한 것을 함께 나르고, **복원 때 문서 인덱스가 그 시점의 Git 과 어긋나게 만든다.**

`--data-only` 인 이유는 스키마의 정본이 `migrations/` 이기 때문이다. DDL 을 함께 뜨면 그 사본이 갈라진다.
복원 뒤 **시퀀스를 손으로 맞추지 않는다** — `pg_dump --data-only --table=kb_events` 가
그 테이블이 소유한 시퀀스의 `setval` 을 덤프 안에 이미 담는다 (2026-09-03 실측).
처음에는 손으로 맞추는 단계를 적었는데, 복원을 실제로 돌려 보고 그것이 덤프가 정한 값을
사람이 다시 정하는 것임을 알았다.

### D55 검사가 쓰는 DB

두 번째 Postgres 를 세우지 않는다 — D13 의 `두 개` 를 다시 해석해야 하고, 얻는 것이 v1 에는 없다.
대신 **격리를 이름으로 한다**: 검사의 `project` 는 전부 `t_` 로 시작하고, 그 밖을 건드리지 않는다.

**그 불변식을 검사가 지킨다** (`tests/test_conventions.py`). 산문으로만 두면 다음 검사가 `sillok` 을 쓰고,
그 순간 검사가 제품 데이터를 지운다. 실제로 이 저장소에서 스모크 주입이 찌꺼기 행을 남긴 적이 있다.

### D56 CI 와 비밀

**CI 를 두지 않는다.** 공개 저장소라 외부 독자가 먼저 찾는 자리이므로 부재를 결정으로 적는다 —
증거는 `evidence.mjs` 한 번으로 손에서 모으고, 그것이 이 저장소의 규모에 맞는다.

비밀은 지금까지 **1회성 실측**뿐이었다 (D26). 게이트에 **검사 17** 을 더해 매번 본다 —
`sk-` 로 시작하는 키 모양, 비밀이 든 DSN, 사설 키 헤더. `.env` 는 `.gitignore` 가 막고,
예외 문구는 D21 이 고정 문자열로 막는다. 셋이 함께 서는 것이지 하나가 다 하지 않는다.

### D57 기여

**받지 않는다.** D15 의 `개인 도구` 가 `plan.md` §2 에 살아 있고, 공개(D26)는 *읽을 수 있게* 한 것이지
*함께 만들자*는 것이 아니었다. 기여 절차·이슈 템플릿·행동 강령을 만들지 않는다 —
만들면 지키지 못할 약속을 문서가 하게 된다.
**두 README 는 이미 그렇게 말하고 있었다** — `기여 절차도, 이슈 템플릿도, 하위호환 약속도 없습니다`.
빠져 있던 것은 문장이 아니라 **결정**이었다. 그 문장이 관행이 아니라 계약이 된다.

### D54–D57 선택지

| 결정 | 버린 안 | 버린 이유 |
|---|---|---|
| D54 대상 | 전체 덤프 | 재생성 가능한 것을 함께 나르고, 복원 시점의 인덱스가 Git 과 어긋난다 |
| D54 자리 | `data-model.md` §운영 확장 | 그 파일은 DDL 의 정본이다. 절차가 섞이면 스키마를 보러 온 사람이 운영 문서를 읽는다 |
| D55 | 두 번째 db 서비스 · 별도 데이터베이스 이름 | D13 의 `두 개` 개정이 필요하고, 마이그레이션을 두 곳에 적용해야 한다 |
| D56 | GitHub Actions | 손에서 도는 증거가 이미 6종이다. CI 를 더하면 그 둘이 갈라지고, 갈라진 쪽이 느슨해진다 |
| D57 | 기여 절차를 만든다 | 지킬 수 없는 약속이다. 받지 않기로 하는 것이 정직하다 |

### D54–D57 이 닫지 않는 것

- **백업을 *언제* 뜨는지는 정하지 않는다.** 주기도 보관 기간도 없다 — 개인 도구의 규모에서
  자동화는 지키지 못할 약속이 된다. 명령이 있다는 것이 이 결정의 전부다
- ~~복원을 실측하지 않았다~~ → **2026-09-03 에 돌렸다.** 이벤트가 든 덤프를 되부어 행과 시퀀스가
  그대로 돌아오는 것을 확인했고, 그 과정에서 **첫 절차가 틀렸다는 것**이 드러났다 —
  `TRUNCATE`·`ON_ERROR_STOP`·`stop api` 없이 부으면 `COPY` 가 기본 키에서 걸리는데도
  `psql` 이 계속 가서 **종료 코드 0 으로 끝난다** (Grok 적대 리뷰가 이 자리를 지목했다).
  덤프가 시퀀스 `setval` 을 이미 담고 있다는 것도 그때 알았다 — 손으로 맞추는 단계를 뺐다
- **검사 17 은 모양만 본다.** 새로 발명되는 비밀 형식은 못 잡는다. `.env` 를 커밋하는 것은
  `.gitignore` 가 막고, 예외로 새는 것은 D21 이 막는다 — 셋이 겹쳐야 한다
- **`t_` 접두사는 파이썬 소스의 네 모양만 본다** — `PROJECT` 상수, `project` 값, `_wipe(...)` 인자,
  SQL `VALUES` 의 첫 칸. 처음에는 상수 하나만 봤고 `_wipe` 가 그물 밖이었다 (Grok 적대 리뷰).
  **계산해서 만든 project 는 여전히 밖이다.** 범위는 `dbcheck` 를 임포트하는 파일이므로
  DB 에 닿는 검사가 새로 생기면 자동으로 따라온다

## D58–D64 — 열린 Q 일곱 (2026-09-03 확정)

§7 1–10단계와 운영이 닫히고 남은 것은 **단계 밖의 질문 일곱**이었다.
Q13·Q14·Q22·Q23·Q24·Q25·Q30 — 어느 것도 단계를 막지 않았고, 그래서 오래 열려 있었다.

**한 줄로 꿰면 이렇다: v1 은 목록 표면을 만들지 않는다.**
다만 그 문장이 Q30 까지 덮지는 않는다 — `GET /v1/docs` 는 D35 가 **지목 조회**로 분류한 것이지
목록이 아니다. 셋을 한 질문으로 묶는 것은 틀렸고, 그 오류는 적대 리뷰가 잡았다.

| ID | 결정 내용 | 닫은 질문 |
|---|---|---|
| D58 | 페이지네이션·목록·타임라인은 v1 밖이다. **모델이 읽는 응답에만** 천장을 준다 | Q13 |
| D59 | 이벤트 수정 경로를 만들지 않는다. 정정은 **새 이벤트**다 | Q14 |
| D60 | `repo` 는 v1 내내 `''` 이고 그것이 규칙이 된다. 응답에 싣지 않는다 | Q22 |
| D61 | `status` 의 생애를 정한다. **Git 쪽 증거이고 스키마는 그대로다** | Q23 |
| D62 | D1–D15 의 선택지는 복원하지 않는다. 대신 D30 이후에 선택지 표를 강제한다 | Q24 |
| D63 | SKILL 헤더에 **본문 해시**를 박는다. 새 표면을 만들지 않는다 | Q25 |
| D64 | `GET /v1/docs` 를 만들지 않고 스케치를 지운다 | Q30 |

### D58 목록은 만들지 않고, 천장은 모델이 읽는 곳에만

컬렉션·커서·타임라인은 v1 밖이다. D12 가 UI 를 이미 뺐고, 이유는 품이 아니라 **토큰 불변식**이다.
**천장이 있는 목록은 페이지네이션이 아니다** — `next` 가 없고 나머지를 달라고 할 방법도 없다.
나머지를 달라는 것이 곧 v1 이 만들지 않는 그 표면이다.

**경계는 MCP 노출이다.** 모델이 읽는 응답에는 천장이 필요하고, 운영자 응답에는 필요 없다 —
아니면 숫자를 발명하게 된다.

| 자리 | 모델이 읽는가 | 결정 |
|---|---|---|
| `event_stats.by_module` | 그렇다 | 12개, `count` 내림 → 키 오름. **`by_module_omitted` 를 함께 낸다** |
| `search_events` 의 `summary` | 그렇다 | `search.clip_excerpt` 를 그대로 쓴다 — 799자 + `…` 로 **합 800자** (D33 §8) |
| `save_event` 의 `payload` | 그렇다 (`get_event` 가 행을 돌려준다, D39) | 아래 식이 2000자 이하. **`summary` 와 같은 숫자**이고 같은 이유다 |
| `ingest` 의 `skipped[]` | **아니다** — `POST /v1/ingest` 는 MCP 도구가 아니다 | **천장 없음.** 운영자는 목록 전체를 원한다 |

**`payload` 의 길이는 한 가지로만 잰다** — `json.dumps(payload, ensure_ascii=False, separators=(",", ":"))`.
구분자를 적지 않으면 기본값이 공백을 넣어 **같은 객체가 재는 사람에 따라 통과와 거절로 갈린다.**
거절 문구는 D25 의 형태를 따른다 — `payload longer than 2000`.
저장된 `jsonb` 는 Postgres 가 정규화하므로 이 수는 **입력을 재는 것**이고, 그 사실을 여기 적는다.

**`by_module_omitted` 가 그 천장을 안전하게 만든다.** D23 은 `sum(by_module) <= total` 이
*module 없는 행이 빠졌다*는 뜻이라고 정했다. 천장만 얹으면 같은 부등식이 두 가지를 한꺼번에
뜻하게 된다. 새 정수가 천장이 떨어뜨린 **키 수**를 말한다. 그것으로 차이가 완전히 풀리지는 않는다 —
떨어진 키들의 **행 수**는 여전히 알 수 없다. 알 수 있는 것은 *천장이 걸렸는가*이고,
`0` 이면 그 부등식이 D23 의 뜻 그대로라는 것이다. 그 이상은 목록 표면이 필요하고 v1 은 만들지 않는다.
**빈 문자열 `module` 은 키가 된다** (D23 이 `""` 를 그대로 저장한다). 그 키도 12 를 함께 센다.
키 정렬은 `COLLATE "C"` 다 — D33 이 경로에 쓴 것과 같은 이유로, 로케일이 순서를 바꾸면 안 된다.

`skipped[]` 에 천장을 두지 않는 것은 **결정이지 누락이 아니다.** 다음 사람이 그것을 고치지 않도록 적는다.

### D59 이벤트 수정 경로

`kb_events` 는 append-only 다 (D24). 수정 경로는 `repeat_causes` 가 센 근거를 나중에 고쳐 쓰게 만든다.
**정정은 그 자체가 이벤트다** — 원장이란 그런 것이다.
[data-model.md](../docs/data-model.md) 의 `v1은 삭제보다 payload/summary 수정` 은 바뀐다: v1 은 둘 다 하지 않는다.

### D60 `repo`

D5 는 v1 에 **project** 를 여럿 준 것이지 repo 를 여럿 준 적이 없다.

- `repo` 는 모든 행에서 `''` 이고 다른 값을 쓰지 않는다. 검사가 그것을 단언한다
- **응답에 싣지 않는다.** 값이 상수인 필드는 소음이고, 지금 실으면 두 번째 repo 가 생기는 날이
  마이그레이션인데도 아무 일 없는 것처럼 보인다
- 정렬 키에는 `repo` 를 남긴다 — 값이 바뀌는 날 순서가 흔들리면 안 된다

### D61 `status` 의 생애 — **Git 쪽 증거이고 스키마는 그대로다**

| 값 | 언제 | 게이트가 요구하는 것 |
|---|---|---|
| `current` | 지금 규범. 기본값 | — |
| `draft` | 아직 계약이 아니다 | 게이트가 더 요구하는 것은 없다 — 도달성은 이미 모든 문서에 걸린다 |
...
자기 자신을 가리키면 실패한다. 사슬은 막지 않는다 — 두 문서가 서로를 가리키는 것은
둘 다 대체됐다는 뜻이고, 그 판단은 사람이 한다.

**두 키는 게이트가 보는 증거이지 컬럼이 아니다.** `kb_documents` 에는 그것을 둘 자리가 없고
새로 만들지도 않는다 — ingest 는 D29 가 정한 네 필드만 읽고 나머지를 버린다 (D30 §7).
DB 는 `status` 만 갖는다. **그 값을 응답에서 보고 싶다면 그것은 스키마 변경이고 다른 결정이다.**

이 저장소의 문서는 전부 `current` 다. 나머지 셋은 **필요해지는 날 규칙이 이미 있도록** 둔다.

**D33 §1 을 뒤집지 않는다.** 그 결정은 `status: "current"` 가 *기본값이 아니라고* 못 박았고
이 결정은 그것을 그대로 둔다 — `status` 를 안 주면 `draft` 도 함께 나온다.
D33 이 그 기본값을 거절한 이유가 `무엇이 언제 stale 이 되는가` 였고 D61 이 그 물음에 답했지만,
**답이 생겼다는 것이 곧 기본값을 만들 이유는 아니다.** 거르고 싶으면 호출자가 `status` 를 준다.
기본 필터를 만들려면 D33 을 개정하는 별도 결정이 필요하다.

### D62 잃어버린 선택지

- D1–D15 의 `A`/`B`/`C` 라벨은 이력으로 남긴다. **복원하지 않는다** —
  복원은 곧 창작이고, 이 파일은 창작이 들어가면 안 되는 하나다
- 규칙: **D30 이후**의 모든 `## Dnn` 절은 `선택지` 또는 `버린 안` 표를 갖는다. 게이트가 검사한다.
  D16–D29 는 규칙보다 앞서므로 **면제 목록에 이름을 적어** 둔다 — 조용히 건너뛰지 않는다.
  면제는 정확히 넷이다: `## D26` · `## D27` · `## D28` · `## D29`.
  나머지(`## D16–D20`·`## D21`·`## D22`·`## D23–D25`)는 이미 표를 갖고 있다.
  검사는 `## D30` 이후의 절만 보고, 절 제목이 `## D35–D38` 처럼 묶여도 **한 절로 센다**

### D63 배포된 SKILL 사본의 노후

`GET /v1/skill` + `kb_status.skill_version` 은 새 표면이고 §7 에 그 단계가 없다.
더 싸고 충분한 것: **사본이 들고 다니는 헤더에 본문의 해시를 박는다.**

- 헤더에 `본문 해시: sha256:<앞 12자리 소문자>` 가 생긴다.
  **대상은 그 줄이 든 인용 블록(`>` 로 시작하는 연속 줄) 다음부터 파일 끝까지**이고,
  D30 의 정규화를 그대로 쓴다(줄 끝을 LF 로, BOM 제거). 그래야 CRLF 작업 트리에서 값이 흔들리지 않는다
- 게이트가 다시 계산해 어긋나면 실패한다 — **원본이 낡은 해시를 들고 나갈 수 없다**
- 사본을 든 사람은 그 한 줄을 공개 저장소와 대조한다. 자동은 아니지만 **경로**이고,
  Q25 가 없다고 한 것이 바로 그 경로다
- **해시를 박는 것과 그것을 지키는 검사는 같은 커밋에서 나간다.** 검사 없이 먼저 박으면
  그 줄은 다음 편집에서 곧바로 낡고, 낡은 해시는 없는 해시보다 나쁘다

### D64 `GET /v1/docs`

**목록이라서가 아니다** — D35 는 그것을 지목 조회로 분류했다(한 경로, 없으면 404).
**§7 이 어느 단계에도 넣지 않았고 소비자가 없어서다:**

- MCP 는 그것을 노출하지 않는다 (D42–D46 이 도구 여덟을 못 박았다)
- 문서를 나열할 UI 는 v1 밖이고 (D12), D58 이 그것을 계속 밖에 둔다
- 그것이 할 지목 읽기는 이미 있다 — `get_file` 이 색인된 문서의 본문을 주고(D36) `search_docs` 가 그것을 찾는다.
  `GET /v1/docs` 는 **같은 방에 세 번째 문**을 다는 일이다

그래서 [plan.md](../docs/plan.md) §5 와 [service-and-mcp.md](../docs/service-and-mcp.md) 에서 지우고
옛 문장을 `RETIRED` 에 등록한다. `api.py` 가 이미 내는 404 가 답의 전부가 된다.
D35 의 표는 *만들었다면* 의 규칙으로 남는다 — 그 표도 단계를 잡은 적이 없다.

`POST /v1/ingest` 는 그 줄에 남는다. 같은 함수의 HTTP 얼굴이기 때문이다 (D20).

### D58–D64 선택지

| 결정 | 버린 안 | 버린 이유 |
|---|---|---|
| D58 | 커서 페이지네이션을 넣는다 | 토큰 불변식이 막는다. `top_k` 를 넘는 결과를 보는 방법이 곧 "관련 문서 전부" 다 |
| D58 `by_module` | 천장만 얹는다 | D23 의 `sum(by_module) <= total` 이 두 가지를 한꺼번에 뜻하게 된다 |
| D58 `skipped[]` | 50개로 자른다 | 형제 숫자가 없어 발명이 된다. 운영자 응답이라 천장이 필요 없다 |
| D59 | `PATCH /v1/events/{id}` | append-only 를 깨고 `repeat_causes` 의 근거를 사후에 바꾼다 |
| D60 | `repo` 에 값을 준다 | 두 번째 repo 가 없는데 값을 정하면 그것이 곧 추측이다. 기존 행 이관도 따라온다 |
| D61 | 두 키를 `kb_documents` 컬럼으로 | D29 의 `front matter 필드 = 컬럼` 1:1 을 깨고 `002` 를 다시 연다 |
| D62 | 선택지를 복원한다 | 복원 불가이고 시도는 창작이다 |
| D63 | `GET /v1/skill` + `skill_version` | 새 표면이고 §7 에 단계가 없다. 사본은 그 표면에 닿지도 못한다 |
| D64 | `GET /v1/docs` 를 만든다 | 소비자가 없다 — MCP 비노출·UI 비범위이고 지목 읽기는 `get_file` 이 한다 |

### D58–D64 가 닫지 않는 것

- **`by_module_omitted` 는 응답 모양을 바꾼다.** 소비자가 없어서 지금은 안전하지만,
  같은 이유로 이 필드가 정말 읽히는지 아무도 모른다
- **`payload` 2000자는 새 거절을 만든다.** 이미 그보다 큰 행이 있다면 그 행은 그대로 남는다 —
  소급하지 않는다
- **`draft`·`superseded`·`stale` 을 쓰는 문서가 아직 없다.** 규칙은 있는데 실물이 없으므로
  그 셋의 게이트 검사는 **고장 주입으로만** 관측된다
- **D63 의 대조는 사람이 한다.** 사본이 스스로 묻지 않는다 — 그 자동화는 D25 가 제안한 표면이고 v1 밖이다
- **D62 의 면제 목록은 손으로 든다.** D16–D29 가 그 안에 있고, 목록이 낡으면 검사가 느슨해진다
...
- MCP 서버를 stdio 와 Streamable HTTP 두 전송으로 한 코드베이스에서 (D6·D45)
...
[open-questions.md](../docs/open-questions.md) I절 Q33 을 마감한다.

D33 은 `plainto_tsquery` 의 AND 가 재현율을 얼마나 깎는지 재지 않은 채 v1 이 그것을
받아들이기로 하면서, 다시 볼 신호를 하나 지정했다 — `hit_count=0 이 쌓이는 것`.
~~키가 들어오자~~ **벡터가 차자** `search_docs` 에서 그 신호가 죽었다 (2026-09-15 정정).
무엇으로 대신하는가가 Q33 이다.

| ID | 선택 | 결정 내용 | 닫은 질문 |
|---|---|---|---|
| D65 | C | **대신할 것을 만들지 않는다.** 거리 임계값도, 키워드 팔 카운터도 두지 않는다. `zero_hit_queries` 가 무엇을 세는지만 정확히 적고, AND 재현율은 **계측하지 않은 채** v1 이 받아들인다 | Q33 |

### 왜 A(거리 임계값)를 버렸나 — 분포가 겹친다

2026-09-05 실측(이 색인 · `text-embedding-3-small`). 벡터 팔 1위 코사인 거리:

```text
주제 안 최악(가장 먼)  0.7548   마이그레이션은 언제 도는가
주제 밖 최선(가장 가까운) 0.6371   --- --- ---
틈 -0.1176 → 겹친다
```

**정당한 질문이 문장부호보다 멀다.** 표본을 32·14·7 로 늘려도 부호가 같다(틈 −0.0851,
문장부호를 빼고 진짜 질문끼리만 재도 그렇다). 임계값을 `0.67` 에 두면 `리액트 훅의 의존성 배열`
과 함께 `질의 원장`(0.6971)·`인증은 어떻게 하는가`(0.6672)가 떨어지고, `0.75` 에 두면
주제 밖이 거의 다 통과한다.

**D33 이 점수 정규화를 버린 이유가 그대로 걸린다** — `절대 기준을 쓰면 그 상수를 정할 근거가 없다`.
거리도 같은 자리다.

### 왜 B(키워드 팔이 빈 질의를 센다)를 버렸나 — 다른 것을 센다

같은 날 실측. 주제 안 아홉 중 여섯이 키워드 0행이다. 그런데 그 여섯의 이유가 갈린다:

```text
이벤트를 어디에 저장하는가   저장하는가=0   (이벤트를=17)  → 조사·어미
검색 결과를 어떻게 병합하는가 병합하는가=0   (검색=38)     → 조사·어미
백업은 무엇을 뜨는가        백업은=0·뜨는가=0 (무엇을=16)   → 조사·어미
마이그레이션은 언제 도는가   전부 있는데 함께 있는 청크가 없다 → 진짜 AND 손실
how are events stored    전부 있다 (`simple` 은 불용어를 안 뺀다) → 진짜 AND 손실
what does ingest index   같다                              → 진짜 AND 손실
```

**절반이 D14(`simple` 구성)의 조사·어미 문제이지 AND 재현율이 아니다.** 이 수를 신호로 삼으면
D33 이 보려던 것과 다른 것을 세게 되고, 벡터 팔이 훌륭한 질의(`이벤트를…` 1위 0.4986)에서도 켜진다.
**신호처럼 보이는데 신호가 아닌 수**를 원장에 더하지 않는다.

덧붙여 D33 이 0건이라 실측해 둔 `임베딩 백필 경로` 는 이제 3행이다 — 전제 자체가 움직였다.

### 그래서 무엇을 하는가

- **`kb_query_logs` 에 컬럼을 더하지 않는다.** D48–D52 는 그대로다.
- **`zero_hit_queries` 가 무엇을 세는지 적는다** — 문서 벡터가 있으면 `search_docs` 는 필터가
  집합을 비울 때만 0건이다. **키만으로는 그 팔이 차지 않는다.** 그 조건을
  [service-and-mcp.md](../docs/service-and-mcp.md)와
  [plan.md](../docs/plan.md) §9 여덟째 항목에 붙인다. 항목을 뒤집지는 않는다:
  `search_events` 와 필터로 여전히 성립한다

- **AND 재현율은 계측하지 않는다.** 다시 볼 때는 **키워드 팔을 직접 재는 것**이 방법이다
  (위 실측이 그 방법이다). 원장이 대리 지표를 쌓아 주길 기다리지 않는다


→ **2026-09-15: `키가 있으면` 은 벡터가 찬 상태의 대용 표기였다.** 키만 있고 `embedding` 이
NULL 이면 필터 없는 0건이 다시 난다 — 키 없이 색인한 뒤 재색인하기 전이 그 창이다.
SQL 이 거르는 단위는 **필터를 통과한 청크의 비NULL 임베딩**이라 `문서 벡터가 있으면` 보다 좁다.
**D65 를 개정하지 않는다** — 대신할 것을 만들지 않는다는 선택은 그대로다.

### D65 선택지

| A | B | C | 버린 이유 |
|---|---|---|---|
| 벡터 팔에 거리 임계값 | `kb_query_logs` 에 키워드 팔 카운터 | 대신할 것을 만들지 않고 뜻만 적는다 | A는 이 색인에서 주제 안·밖 분포가 겹쳐 상수를 정할 근거가 없다 (D33 이 정규화를 버린 그 이유). B는 절반이 조사·어미 문제라 AND 재현율이 아닌 것을 세고, 벡터가 좋은 질의에서도 켜진다 — 신호처럼 보이는 수가 더 나쁘다 |

### D65 가 닫지 않는 것

- **AND 재현율의 크기를 여전히 모른다.** 실사용 질의 분포가 없다는 D33 의 말은 그대로다.
  달라진 것은 `hit_count=0` 을 기다리면 안 된다는 것뿐이다
- **D14 의 조사·어미 손실을 재지 않았다.** 위 여섯 중 셋이 그것인데 크기는 모른다.
  형태소 분석기는 D14 개정이고 v1 밖이다
- **이 결과는 이 색인의 것이다.** 문서 열한 개 · `text-embedding-3-small` 이다.
  코퍼스가 커지면 분포가 갈라질 수 있고, 그때 A 를 다시 볼 수 있다

## D66 — Q34 의 답: 복제 단위는 `api` 가 아니라 스택 전체다 (2026-09-12 확정)

[open-questions.md](../docs/open-questions.md) J절 Q34 를 닫는다. **단계를 막지 않는다** — 열 단계는 이미 구현됐다.

| ID | 선택 | 결정 내용 | 닫은 질문 |
|---|---|---|---|
| D66 | B | D37 의 `인스턴스를 여럿` 은 **자체 `DATABASE_URL` 을 갖는 스택 전체**다. 같은 DSN 에 `api` 만 여럿 두지 않는다. 기본 `up` 은 `db`+`api` 둘 그대로이고 **D13 을 개정하지 않는다** | Q34 |

### 선택지

| ID | A | B | C | 버린 이유 |
|---|---|---|---|---|
| D66 | 같은 DSN 에 `api` 만 project 마다 | **스택 전체 복제 (자체 DSN·나무·포트)** | workspace 뿌리를 두 저장소의 부모로 | A·C 는 아래 실측이 버렸다 |

### A 를 버린 실측

같은 DSN 에 나무가 다른 `api` 를 두고 쟀다. **세 가지가 났다.**

- **색인 파괴.** 가짜 나무 인스턴스가 같은 project 로 ingest 하니 `files_deleted` 10 · `chunks_upserted` 1 이었고
  공유 색인이 `documents` 11→1 · `chunks` 333→1 이 됐다. **오답이 아니라 삭제다.**
- **검색과 원문의 분열.** 그 뒤 한 인스턴스 안에서 `search_docs` 는 한쪽 나무의 청크를,
  `get_file` 은 다른 쪽 나무의 본문을 돌려준다. **두 얼굴에 같은 요청을 보내 `save_doc` 도 쟀다** —
  둘 다 200 이고 `diff` 의 `-` 줄이 각자 제 나무의 본문이었다.
- **동시 기동 실패.** 빈 DB 를 공유하는 인스턴스 둘을 동시에 띄우면 한쪽이 `exit 1` 로 죽는다 —
  `UniqueViolation ... "pg_type_typname_nsp_index" ... (typname, typnamespace)=(kb_documents, 2200)`.
  **마이그레이션은 재기동에 멱등이지만 동시 최초 적용에 안전하지 않다.** 이미 적용된 DB 에서는 둘 다 산다.

**원장은 안 지워졌다** — 같은 시행에서 `events` 1→1, `get_event` 200, `event_stats` 1, 검색 1건이었다.
색인은 재생성 가능하고 원장은 Git 에 원본이 없는 유일한 데이터다 (D11). 그 둘의 위험이 다르다.

### C 를 버린 실측

부모 디렉터리를 `SILLOK_WORKSPACE` 로 삼으니 ingest 가 `status: "failed"` · `files_seen: 0` 으로 끝났고
`documents` 0 · `chunks` 0 이며 세 경로 모두 404 였다. **기능 상실이지 해소가 아니다.**

### B 가 지키는 것

- 자체 DB·나무·포트로 띄운 스택은 제 나무를 열고, 생산 색인·원장에 아무것도 남기지 않는다
- 분리 스택 둘을 **동시에** 기동해도 `UniqueViolation` 이 나지 않는다
- 한 스택이 색인하는 동안 다른 스택은 정상 응답한다 — 아래 참조

### 왜 이것이 성능 문제이기도 한가

**이 소절은 결정이 아니라 그때의 관측이다.** D66 이 고른 값은 복제 단위(스택 전체)이고 그 근거는
위 A·C 의 색인 파괴·검색과 원문의 분열·동시 마이그레이션 실패다. 아래 막힘은 그 값이 아니다.

~~**ingest 가 도는 동안 그 인스턴스의 표면이 전부 막힌다.**~~ 임베딩이 도는 ingest 중에
`GET /v1/status` · `GET /v1/files` · `POST /v1/search/docs` · `GET /v1/stats/events` · `POST /v1/events` ·
`POST /mcp` 여섯이 모두 15초 안에 응답하지 않았고, ingest 가 끝난 뒤 같은 호출은 밀리초로 돌아왔다.
~~**메커니즘은 재지 않았다** — 관측은 여기까지다.~~

~~**색인하는 project 와 막히는 질의의 project 는 서로 달랐다.**~~ 한 인스턴스에 project 둘을 두고
한쪽을 색인하는 동안 **다른 쪽 project 를 물었더니** 그 여섯이 막혔다 — 같은 project 를 물어서가 아니다.

같은 순간 **DB 와 나무가 분리된 다른 스택은 전부 정상 응답했다.** ~~스택을 나누면 그 차단이 넘어가지 않는다.~~

→ **2026-09-13 에 그 메커니즘을 쟀다.** 막은 것은 락이 아니라 **`POST /v1/ingest` 가 `await` 없이
`async def` 라 그 run 동안 이벤트 루프를 잡은 사고**였다. `/mcp` 의 도구는 원래 `def` 라 루프만 비면 답한다.
업무 라우트 아홉을 `def` 로 두니 같은 인스턴스의 다른 표면이 ingest 중에도 평시 속도로 답했다.
**D66 을 개정하지 않는다** — 여러 저장소를 섬기려면 여전히 스택 전체를 복제한다. 이 막힘은 그 이유가 아니었다.

### 락은 그대로다

같은 DSN·같은 project 로 **프로세스 둘**이 동시에 ingest 하면 한쪽 200, 다른 쪽 `409 CONFLICT` 다 (D32 그대로).
~~한 인스턴스에 같은 요청 둘을 보내면 뒤엣것이 대기 뒤 200 에 임베딩 0 으로 끝나고 409 는 나오지 않는다 —
그 대기가 락 때문인지 다른 병목인지는 이 관측으로 갈리지 않는다.~~ 다른 project 끼리는 둘 다 200 이다.

→ **2026-09-13: 같은 프로세스 안에서도 한쪽 200, 다른 쪽 409 다.** 루프가 요청을 겹치지 않게 세워
락이 닿을 일이 없었던 것이고, 라우트를 `def` 로 두니 **D32 가 한 프로세스에서 처음으로 도달한다.**

**수는 여기 적지 않는다.** run 번호·바이트·시간은 `search_events` 의 `project=sillok` · `module=api` 와
`module=ingest` 에 있다.
...
D67 이후로 기록해야 할 미해결 결정은 [docs/open-questions.md](../docs/open-questions.md)에 전부 모여 있다.
...
D33이 지정한 `hit_count=0` 신호가 `search_docs` 에서 죽는 것을 무엇으로 대신할지가 그 질문이었고,
**답은 `대신하지 않는다`다** — 두 후보를 실측이 다 버렸다. **열린 항목은 다시 하나도 없다.**
**Q31은 열어 둔 이튿날 D47로 닫혔다** — 8단계가 `.venv` 에 서드파티 문서를 들여 게이트를 붉게 만들었다.
Q23 은 D29 가 절반만 답했고 **나머지 절반을 D61 이 닫았다** — 값을 어디서 얻는지에 더해
`status` 의 생애까지 정해졌다.
