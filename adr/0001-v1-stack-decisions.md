...
- **검색과 원문의 분열.** 그 뒤 한 인스턴스 안에서 `search_docs` 는 한쪽 나무의 청크를,
  `get_file` 은 다른 쪽 나무의 본문을 돌려준다. `save_doc` 의 `diff` 도 제 나무를 기준으로 갈라진다.
...
같은 순간 **DB 와 나무가 분리된 다른 스택은 전부 정상 응답했다.** 그래서 한 인스턴스로 여러 project 를
섬기면 한 project 의 색인이 나머지 전부를 멈추고, 스택을 나누면 그 차단이 넘어가지 않는다.
