...
- ~~의존성 변경 시 재빌드를 강제하는 검사는 없다~~ → **2026-09-03 에 검사가 생겼다.**
  `tests/test_dependencies.py` 가 설치된 버전과 `uv.lock` 을 대조하고, compose 의 `test` 서비스가
  잠금 파일을 마운트한다. **새 임포트는 `ImportError` 로 시끄럽지만 버전 상향과 제거는 조용했다** —
  그 자리를 막는다. 실측: 잠금 파일을 갈라 놓고 컨테이너에서 돌리니
  `httpx: 설치 0.28.1 vs 잠금 9.9.9` 로 붉어졌다
