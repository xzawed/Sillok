...
4. 적대적 리뷰를 받는다. 리뷰는 diff만 읽지 않고 라이브를 때린다.
   ...
   **리뷰 요지는 `gh pr comment`로 그 PR에 붙인다** — 무엇을 지적했는지, BLOCKER였는지,
   무엇으로 잠갔는지. 커밋 메시지에만 남기면 **나중에 PR만 읽는 감사에게는 리뷰가 없어 보인다** —
   `PR #1`–`#13`과 `PR #39`–`#45`의 GitHub `reviews`가 전부 비어 있고, 같은 지적이 두 번 났다.
...
- `smoke.test.mjs` — 10단계 스모크가 무는지 (D53). **스모크 자신은 증거가 아니다** —
  살아 있는 `api` 를 요구하고, 그것은 v1 완료 판정이지 PR 하나의 증거가 아니다
...
단위를 뗀 `N 통과`나 전각 숫자 `５５１ passed` 가 그대로 지나가는 것을 고장 주입으로 확인했다.
목록을 늘려 잠글 자리가 아니다 — 같은 뜻의 표기는 얼마든지 만들 수 있다.

§9는 **v1의** 완료 조건이지 PR 하나의 완료 조건이 아니다.

### 머지된 이력의 정정

**커밋 메시지는 게이트 밖이고, 나간 것은 되쓸 수 없다.** 그래서 정정을 여기에 남긴다 —
이력을 근거로 인용하는 다음 감사가 틀린 수를 그대로 물려받지 않게.

2026-09-06 감사가 셋을 찾았고 **전부 숫자였다. 행위 주장은 하나도 틀리지 않았다.**

| 커밋 | 메시지에 적힌 것 | 실제 |
|---|---|---|
| `e91be4f` (`#42`) | 제목·본문의 `열셋`, 소제목 `나머지 여섯` | 그 커밋이 고친 것은 **열넷**이다 — `ADR (일곱)` 아래 일곱, `나머지 여섯` 아래 **일곱**(여섯이라 적고 일곱을 열거했다). 후속 커밋의 셋 중 `D37` 하나만 새 자리라 **열다섯**이다 — `D47 귀속`은 그 커밋 자신을 고친 것이고 `5단계 전`은 이미 센 주장의 셋째 **사본**이다 (`d0ccded` 둘째 커밋이 이 접기를 확정했다) |
| `e357dcf` (`#44`) | `열셋 중 하나`(F13 회수율) · `열셋 중 둘`(검사 21) | 분모는 **열다섯**이다. 검사 21은 그때 반쪽(`Dnn 이후로` 만)이라 **하나**만 잡았고, `d0ccded`가 나머지 반쪽(`확정. 임의로 뒤집지 않는다` 열거)을 만들어 둘이 됐다 |
| `f39017c` (`#41`) | `모든 명령이 성공을 보고하면서 원장이 없어졌다` | **같은 메시지의 실측 블록이 `test -s 종료코드: 1`이라 적고 있다.** 종료 코드 `0` 을 보고한 것은 그 뒤 **둘**(`TRUNCATE`·`붓기`)이고, `남은 이벤트: 0` 은 성공 보고가 아니라 피해 측정이다 |

문서 쪽은 `d0ccded`가 이미 고쳤다 — `docs/conventions.md`의 회수율 둘이 `열다섯`이다.

**그렇다고 커밋 메시지에서 수치를 빼지는 않는다.** 실측한 수가 이 저장소에서 실제로 결함을 증명했다 —
`f39017c`의 `남은 이벤트: 0`이 조용한 실패를 눈에 보이게 만든 것이 그 수다.
틀리는 자리는 **세는 규칙을 중간에 바꿀 때**다. 사본을 접어 세다가 한 사본을 새 항목으로 다시 셌고,
그래서 한때 열여섯으로 셌다 — **그 열여섯이 오류 경로이고 정정값은 열다섯이다.**
**세기 전에 접을지 말지를 못 박는다.**

2026-09-15 감사가 그 뒤 구간(`d0ccded`~`209d2d5`, 열여섯 커밋)을 훑어 **셋을 더 찾았다.
이번에도 전부 숫자이고, 셋 다 같은 모양이다 — 쓴 수보다 열거가 하나 많다.**
2026-09-06 의 `e91be4f` 가 `여섯이라 적고 일곱을 열거했다` 던 그 오류다.

| 커밋 | 메시지에 적힌 것 | 실제 |
|---|---|---|
| `de5529b` (`#52`) | `같은 리뷰의 닛 셋도 고쳤다` | 그 문단이 **넷**을 나열한다 — `bash` 펜스 안의 MCP 입구, `본 파일이 없다`, `자기 라벨로 색인하면`, 오타 하나 |
| `2887ff1` (`#58`) | 둘째 커밋 제목·본문의 `내가 새로 만든 문제 셋` | 같은 본문이 **넷**을 열거한다 — 태그라인 중복, `Git 저장소 옆에서`, `모델에게 행을`, 비용 문장의 두 README 갈라짐 |
| `209d2d5` (`#60`) | 첫 커밋의 `같은 대용 표기가 다섯 곳에 있었다` | 그 문장이 열거한 것이 **여섯**이다. 리뷰 커밋의 `두 곳 더` 는 맞으므로 브랜치 합계는 **여덟**이다 |

**행위 주장은 이번에도 하나도 틀리지 않았다.** `#59` 의 `async def 아홉` 은 정확히 아홉이고,
`#55` 의 `넷과 하나` 산술도 맞다. 틀리는 자리가 **열거와 숫자를 따로 쓸 때**라는 것이
2026-09-06 과 같다 — 그때는 세는 규칙을 중간에 바꿔서, 이번에는 열거를 고친 뒤 앞의 수를 안 고쳐서다.
**열거를 손대면 그 문장의 수를 다시 센다.**
