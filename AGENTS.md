...
2. 정본(plan·ADR) → 하위 문서 → 코드. 역순으로 하지 않는다.
3. 구현한다.
4. **살아 있는 것을 때린다.** 새로 생긴 표면을 실제로 호출한 출력이 증거다.
5. 적대적 리뷰를 받는다. 리뷰는 diff만 읽지 않고 라이브를 때린다.
   **BLOCKER**는 계약 위반(공통 봉투, 불변식, 비밀 유출). 나머지는 닛이다.
6. 지적은 **고장을 재현하는 테스트로 잠근다.** 고치고 끝내지 않는다.
7. BLOCKER를 고쳤으면 **재검토를 받고** 머지한다. 무시하고 머지하지 않는다.
8. 머지까지가 끝이다. PR을 열어 둔 상태는 종료가 아니다.
...
- `uv run pytest -p no:warnings` — 통과 개수와 **skip 개수**까지. skip이 실패를 가릴 수 있다
...
- **명세에 이미 답이 있는 순수 로직은 테스트를 먼저 쓴다.**
  `error()`의 `INTERNAL` 고정은 D21에 적혀 있었는데 구현이 어겼고 리뷰가 잡았다. 테스트 먼저였으면 먼저 걸렸다.
- **프레임워크 기본값·DB·HTTP 경계는 테스트 먼저로 도달하지 못한다.** 라이브 고장 주입으로 찾고 그 자리에서 잠근다.
  `/openapi.json`이 살아남은 것, 빈 본문 307, 중복 `Authorization`, 비-ASCII `compare_digest` — 전부 명세에 문장이 없던 것들이다.
- 검사를 만들면 통과만 보지 말고 **고장을 주입해 실제로 실패하는지** 확인한다. 통과 출력만으로는 검사가 살아 있는지 알 수 없다.
