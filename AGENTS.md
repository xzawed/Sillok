...
| 커밋 | 메시지에 적힌 것 | 실제 |
|---|---|---|
| `e91be4f` (`#42`) | 제목·본문의 `열셋`, 소제목 `나머지 여섯` | 그 커밋이 고친 것은 **열넷**이다 — 여섯이라 적고 일곱을 열거했다. 후속 하나(`D37` 자리)를 더해 **열다섯** |
| `e357dcf` (`#44`) | `열셋 중 하나`(F13 회수율) · `열셋 중 둘`(검사 21) | 분모는 **열다섯**이다. 검사 21은 그때 반쪽이라 **하나**만 잡았고, `d0ccded`가 나머지 반쪽을 만들어 둘이 됐다 |
| `f39017c` (`#41`) | `모든 명령이 성공을 보고하면서 원장이 없어졌다` | **같은 메시지의 표가 `test -s 종료코드: 1`이라 적고 있다.** 성공을 보고한 것은 그 뒤 셋이다 |
