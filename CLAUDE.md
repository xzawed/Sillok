...
작업 순서 1–10단계를 이제 검증할 수 있다 — **계약이 다 있다는 뜻이지 구현이 다 됐다는 뜻이 아니다.**
**단계를 막는 Q는 하나도 남지 않았다.**

**G절(Q30·Q31)은 그 7단계를 검증하다 열렸고 아직 답이 없다** — `GET /v1/docs`의 단계·계약,
D30 전체 스캔의 비용. 단계를 막지는 않지만 그 표면을 건드리기 전에 답한다.
