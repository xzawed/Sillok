...
Q17이 **D42–D46**(8단계 MCP 표면)으로 마감되면서
작업 순서 1–10단계를 이제 검증할 수 있다 — 계약이 다 있다는 뜻이고,
...
**I절의 Q33은 2026-09-04에 열렸고 이튿날 D65가 닫았다** — 키가 있으면 D33이 지정한
`hit_count=0` 신호가 `search_docs` 에서 죽는다. **답은 `대신하지 않는다`다** —
거리 임계값은 분포가 겹쳐 상수를 못 정하고, 키워드 팔 카운터는 절반이 조사·어미 문제를 센다.
