...
**열린 Q 는 없다** — Q34 까지 D66 이 닫았다. `GET /v1/docs` 는 만들지 않기로 정했다 (D64).
...
J절의 Q34까지 D66이 닫았다.
새 공백을 발견하면 추측으로 채우지 말고 거기에 다음 번호로 열고, 답을 ADR에 D67 이후로 적는다.

어느 Q가 어느 D로 닫혔는지는 [docs/open-questions.md](docs/open-questions.md)가 소유한다 —
여기에 그 이력의 사본을 두지 않는다.
**단계를 막는 Q는 하나도 남지 않았다** — 작업 순서 1–10단계를 이제 검증할 수 있다.
계약이 다 있다는 뜻이고 그 열 단계의 구현도 끝났다 (진행 상태 정본은 §7·§9다).

**아래 셋은 이력이 아니라 지금 도는 규칙이라 미러로 남긴다.**

- **D65** — 문서 벡터가 있으면 D33이 지정한 `hit_count=0` 신호가 `search_docs` 에서 죽는다.
  **답은 `대신하지 않는다`다** — 거리 임계값은 분포가 겹쳐 상수를 못 정하고,
  키워드 팔 카운터는 절반이 조사·어미 문제를 센다
- **D48–D52** — 로그를 남기는 표면은 검색 둘, `client` 는 `http`·`mcp`, `hit_count` 는 돌려준 행 수다
- **D66** — 여러 저장소를 섬기려면 **스택 전체**를 복제한다 (자체 `DATABASE_URL`·나무·포트).
  같은 DSN 에 `api` 만 여럿 두지 않는다 — 실측에서 색인이 지워지고 검색과 원문이 갈라졌다.
  기본 `up` 은 `db`+`api` 둘 그대로다 (D13 개정 없음)
