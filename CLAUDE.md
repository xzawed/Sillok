...
**코드가 아직 없다.** 이 폴더는 Sillok 구현을 위한 명세 묶음이고, 문서 자체가 계약이다.
빌드·테스트·린트 명령이 존재하지 않는 것은 정상이다 (구현 전 단계, Git 저장소도 아님).

읽는 순서는 `00-README.md`에 있고, **시작점은 항상 `PLAN.md`다.**

### 문서 우선 (가장 중요한 규칙)

- 동작이 명세와 다르면 **코드가 틀린 것으로 본다.**
- 계약을 바꾸려면 `PLAN.md`와 `05-OPEN-DECISIONS.md`를 **먼저** 고치고 나서 구현한다.
- 문서 간 충돌 시 우선순위: `PLAN.md` = `05-OPEN-DECISIONS.md` > `01`~`04`.
- `05-OPEN-DECISIONS.md`의 D1–D15는 2026-08-30 확정. 임의로 뒤집지 않는다.

## 핵심 불변식

Sillok은 RAG 플랫폼이 아니라 **저장 위치를 강제하는 지식 원장**이다. 모든 설계 판단은 여기서 나온다.
...
분류 판단이 필요할 때 감으로 하지 않고 `02-STORAGE-RULES.md`의 결정 트리를 따른다.
이 파일은 동시에 프로젝트에 복사되는 Skill 본문이기도 하다.
...
임베딩 모델을 바꾸면 스키마 변경 + 전체 재색인이 따라와야 한다 (차원이 DDL에 박혀 있음).
...
출구              MCP 도구 8개 + Skill(02-STORAGE-RULES.md) + JSON 현황 API
```
...
입출력 JSON 전문은 `04-SERVICE-AND-MCP.md`. MCP 도구 설명문은 짧게 — 길면 모델이 도구를 안 고른다.
...
`PLAN.md` §7. 요지는 **임베딩 없이 도는 것부터** 세운다는 것:

1. Compose (Postgres + pgvector) → 2. 마이그레이션(`03-DATA-MODEL.md` DDL 그대로)
...
- 이벤트 이력을 Git 문서(`docs/`)에 append — 이 경로가 존재하면 프로젝트가 실패한 것
...
- Grok: 명세 준수, 스키마/계약, 리뷰 · Claude: 구현/테스트 — 권장 분담
- 문서를 바꾸면 다른 쪽에 변경 요약을 남긴다
- 식별자(테이블·도구·필드)는 영어, 설명 문장은 한국어 허용
